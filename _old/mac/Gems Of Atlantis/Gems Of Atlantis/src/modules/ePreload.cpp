...
int ePreload::Render(unsigned long ulTimer, CMouse *pMouse)
{
	...
	if ((ulTimer - m_ulTimerStart)<200)
	{
		m_pBlack->fAlfa(1.0f-((ulTimer-m_ulTimerStart)*0.005f));
		m_pBlack->Render();
	}

	if ((ulTimer - m_ulTimerStart)>499 && (ulTimer - m_ulTimerStart)<754)
	{
		m_pBlack->fAlfa(1.0f-(ulTimer - m_ulTimerStart-499)*0.005f);
		m_pBlack->Render();
	}
