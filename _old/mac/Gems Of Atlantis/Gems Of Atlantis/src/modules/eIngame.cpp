...
{	
	...
	m_pParticleManager = new CParticle2DManager((char*)"particle.png", 250, 
                                               -100,-100,1024,768,
	...
	if (pScreen->iSizeX()==1024 && pScreen->iSizeY()==600) _uiBoardOffsetY = 0;
	if (pScreen->iSizeX()==1024 && pScreen->iSizeY()==768) _uiBoardOffsetY = 64;
	if (pScreen->iSizeX()==1280 && pScreen->iSizeY()==600) _uiBoardOffsetY = 0;
	if (pScreen->iSizeX()==1280 && pScreen->iSizeY()==720) _uiBoardOffsetY = 0;
	if (pScreen->iSizeX()==1280 && pScreen->iSizeY()==800) _uiBoardOffsetY = 64;
	if (pScreen->iSizeX()==1366 && pScreen->iSizeY()==768) _uiBoardOffsetY = 64;
	if (pScreen->iSizeX()==1920 && pScreen->iSizeY()==1080) _uiBoardOffsetY = 128;
	if (pScreen->iSizeX()==2560 && pScreen->iSizeY()==1440) _uiBoardOffsetY = 128;
	if (pScreen->iSizeX()==2048 && pScreen->iSizeY()==1536) _uiBoardOffsetY = 128;
...
void eIngame::DrawProgressBar(unsigned long ulTimer)
{
	...
	else if (_pScreen->iSizeX()==1024 && _pScreen->iSizeY()==600)
	{
		iTimeX1 = 375; iTimeY1 = 495;
		iTimeX2 = 655; iTimeY2 = 516;

		iExpX1 = 375; iExpY1 = 528;
		iExpX2 = 655; iExpY2 = 549;
	}
...
int eIngame::Render(unsigned long ulTimer, CMouse *pMouse)
{
	...
	{
		h1 = (int)((pMouse->fMouseX()-((_pScreen->iSizeX()/2)-(m_pBoard->fSizeX()/2)))/64);	// X na tablicy
		h2 = (int)((pMouse->fMouseY()-_uiBoardOffsetY)/64);	// Y na tablicy
	...
	for (h1=0; h1<=7; h1++) // czy mozliwe ruchy w poziomie
	{
		for (h2=0; h2<=6; h2++)
		{
				...
				m_cBoard[(h1*8)+h2]<GEM_ROCK && m_cBoard[(h1*8)+h2+1]<GEM_ROCK)	// sprawdzamy tylko kamyki standardowe
			{
				...
				{
					c2=1;
					m_iHintX = ((_pScreen->iSizeX()/2)-(m_pBoard->fSizeX()/2)) + (h2*m_pGem[0]->fSizeX());
					m_iHintY = _uiBoardOffsetY + (h1*64);
				}
	...
	if (_pScreen->iSizeX()==1024 && _pScreen->iSizeY()==600) h3=16;
