...
int ePostGame::Render(unsigned long ulTimer, CMouse *pMouse)
{
	...
	{
		x = 170; xx = 950;	// wspolrzedne dla punktow
		y = 16;
		yy = 650;					// poziom 
		iBarY = 100;
		iKerning = 32;
	...
	{
		x = 220; xx = 1200;	// wspolrzedne dla punktow
		y = 64;
		yy = 800;					// poziom 
		iBarY = 200;
		iKerning = 64;
	...
	{
		x = 220; xx = 2560 - 600;	// wspolrzedne dla punktow
		y = 64;
	...
	for (int i=0; i<8; i++)
	{
		m_pNumber[cToDigit(cScore[i])]->Position(x+(iKerning*i), y);
	...
	for (int i=0; i<8; i++)
	{
		m_pNumber[cToDigit(cScore[i])]->Position(xx+(iKerning*i), y);
		m_pNumber[cToDigit(cScore[i])]->Render();
	}
	...
	for (int i=0; i<10; i++)
	{
		m_pFont->Print(_pScreen->iSizeX()/8,iBarY+iKerning+iKerning+(i*(iKerning*0.75f)),255, fSize, fSize, m_cNameTable[i]);
		sprintf(cUserName,"%d",m_ulScoreTable[i]);	// wykorzystujemy nie uzywana juz zmienna do konwersji liczby na ciag znakow
		m_pFont->Print(_pScreen->iSizeX() - (_pScreen->iSizeX() / 4), iBarY + iKerning+iKerning + (i*(iKerning*0.75f)), 255, fSize, fSize, cUserName);
	}
