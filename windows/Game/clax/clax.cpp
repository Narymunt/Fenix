...
struct {
  ...
  {(char*)"3DS", clax_load_mesh_3DS, clax_load_motion_3DS, NULL},
  {(char*)"CLX", clax_load_mesh_CLX, clax_load_motion_CLX, clax_save_CLX}
};
...
*/
  switch (code) {
    case clax_err_nomem:     return (char*)"not enough memory";
    case clax_err_nofile:    return (char*)"file not found";
    case clax_err_badfile:   return (char*)"corrupted file";
    case clax_err_badver:    return (char*)"unsupported version";
    case clax_err_badformat: return (char*)"unsupported format";
    case clax_err_badframe:  return (char*)"invalid frame number";
    case clax_err_badname:   return (char*)"invalid object name";
    case clax_err_noframes:  return (char*)"no frames in keyframer";
    case clax_err_notloaded: return (char*)"scene not loaded";
    case clax_err_nullptr:   return (char*)"null pointer assignment";
    case clax_err_invparam:  return (char*)"invalid parameter";
    case clax_err_spline:    return (char*)"less than 2 keys in spline";
    case clax_err_singular:  return (char*)"cannot inverse singular matrix";
    case clax_err_badid:     return (char*)"bad object id";
    case clax_err_exist:     return (char*)"object already exist";
  ...
  for (node = clax_scene->world; node; node = node->next) {
    switch (node->type) {
      ...
      case clax_obj_material:
        ...
        print_map (&mat->texture, (char*)"texture map:\n");
        print_map (&mat->bump, (char*)"bump map:\n");
        print_map (&mat->reflection, (char*)"reflection map:\n");
        printf ("\n");
        break;
      case clax_obj_ambient:
        amb = (c_AMBIENT *)node->object;
  ...
  for (node = clax_scene->keyframer; node; node = node->next)
    switch (node->type) {
      case clax_track_camera:
        ...
        print_track (cam->pos, (char*)"  position keys:", 1);
        print_track (cam->roll, (char*)"  roll keys:    ", 0);
        print_track (cam->fov, (char*)"  fov keys:     ", 0);
        printf ("\n");
        break;
      case clax_track_cameratgt:
        ...
        printf ("<<camera target>> id: %d, name: %s\n", node->id,
                ((c_CAMERA *)node->object)->name);
        print_track (tcam->pos, (char*)"  position keys:", 1);
        printf ("\n");
        break;
      case clax_track_light:
        light = (t_LIGHT *)node->track;
        ...
        print_track (light->pos, (char*)"  position keys:", 1);
        print_track (light->color, (char*)"  color keys:   ", 2);
        printf ("\n");
        break;
      case clax_track_spotlight:
        spotl = (t_SPOTLIGHT *)node->track;
        printf ("<<spotlight source>> id: %d, name: %s\n", node->id,
                ((c_LIGHT *)node->object)->name);
        print_track (spotl->pos, (char*)"  position keys:", 1);
        print_track (spotl->color, (char*)"  color keys:   ", 2);
        print_track (spotl->roll, (char*)"  roll keys:    ", 0);
        printf ("\n");
        break;
      case clax_track_lighttgt:
        ...
        printf ("<<light target>> id: %d, name: %s\n", node->id,
                ((c_LIGHT *)node->object)->name);
        print_track (tlight->pos, (char*)"  position keys:", 1);
        printf ("\n");
        break;
      case clax_track_object:
        obj = (t_OBJECT *)node->track;
        ...
        print_track (obj->translate, (char*)"  position keys:", 1);
        print_track (obj->rotate, (char*)"  rotation keys:", 3);
        print_track (obj->scale, (char*)"  scale keys:   ", 1);
        print_track (obj->morph, (char*)"  morph keys:   ", 4);
        print_track (obj->hide, (char*)"  hide keys:    ", 5);
        printf ("\n");
        break;
      case clax_track_ambient:
        amb = (t_AMBIENT *)node->track;
        ...
        print_track (amb->color, (char*)"  color keys:   ", 2);
        printf ("\n");
    }
